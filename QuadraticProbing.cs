...
// A hash table class that uses quadratic probing for hashing
    ...
    where T : IRegistry<T>
{
    ...
    /*
     * +---------------------------------------------------------------+
     * | Determines the hash value of a key based on quadratic probing |
     * +---------------------------------------------------------------+
     * 
     * If the hash table is (a) more than half full or (b) the position corresponding to
     * the initial hash value is empty, immediately returns the initial hash value.
     * 
     * (a)
     * It only makes sense to apply quadratic probing if the hash table is less than half full.
     * This condition guarantees that an empty position will be found using quadratic probing.
     * If this condition is not met, it is not possible to determine if either an empty position will be found
     * or its search will ever cease.
    */
